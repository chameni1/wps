...
# Rev 1
import re
import cmsdb.basic as cms
def check(url, ua):
    robots = url + '/robots.txt'
    robots_source = basic.getsource(robots, ua)
            ...
            if ':-' in detection_key:
                ...
                if '||' in detection_array[0]:
                    ...
                    for detection_string in detection_strings:
                        if detection_string in robots_txt_content and detection_array[1] not in basic.ignore_cms:
                    ...
                    for match_string in match_strings:
                        if match_status == '0' or match_status == '1':
                            ...
                            match_status = '2'
                    if match_status == '1' and detection_array[1] not in basic.ignore_cms:
                        if basic.strict_cms == [] or detection_array[1] in basic.strict_cms:
                            return ['1', detection_array[1]]
                else:
                    if detection_array[0] in robots_txt_content and detection_array[1] not in basic.ignore_cms:
                        if basic.strict_cms == [] or detection_array[1] in basic.strict_cms:
                            return ['1', detection_array[1]]

        t3_regex = re.search(r'Sitemap: http(.*?)\?type=', robots_txt_content)
        if t3_regex != None and 'tp3' not in basic.ignore_cms:
            if basic.strict_cms == [] or 'tp3' in basic.strict_cms:
                return ['1', 'tp3']

        return ['0','']
    else:
        basic.error('robots.txt not found or empty!')
        return ['0','']
