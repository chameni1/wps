...
def start(id, url, ua, ga, source):
    version = '0'
    basic.info('detecting joomla version')

    # version detection stats here
    if ga == '1':
        # Detect version via generator meta tag
        basic.statement('Detecting version using generator meta tag [Method 1 of 4]')
        regex_1 = re.findall(r'content=(?:\"|\')Joomla! (.*?) - Open Source Content Management(?:\"|\')', source)
        if regex_1 != []:
            basic.success('Joomla version detected, version: ' + basic.bold + regex_1[0] + basic.cln)
            return regex_1[0]
    ...
    if version == '0':
        ...
        basic.statement('Detecting version using xml files [Method 2 of 4]')
        for xml_file in xml_files:
            xml_source = basic.getsource(url + '/' + xml_file, ua)
            if xml_source[0] == '1':
                regex_2 = re.findall(r'<version>(.*?)</version>', xml_source[1])
                if regex_2 != []:
                    basic.success('Joomla version detected, version: ' + basic.bold + regex_2[0] + basic.cln)
                    return regex_2[0]
    ...
    if version == '0':
        ...
        basic.statement('Detecting version using advanced fingerprinting [Method 3 of 4]')
        for file in other_files:
            file_source = basic.getsource(url + '/' + file, ua)
            if file_source[0] == '1':
                # Regex find
                regex_3 = re.findall(r'<meta name="Keywords" content="(.*?)">', file_source[1])
                if regex_3 != []:
                    basic.success('Joomla version detected, version: ' + basic.bold + regex_3[0] + basic.cln)
                    return regex_3[0]
                ...
                for j in j16:
                    ...
                    if rsearch is not None:
                        basic.success('Joomla version detected, version: ' + basic.bold + '1.6' + basic.cln)
                        return '1.6'
                ...
                for j in j15:
                    rsearch = re.search(j,file_source[1])
                    if rsearch is not None:
                        basic.success('Joomla version detected, version: ' + basic.bold + '1.5' + basic.cln)
                        return '1.5'
                ...
                for j in j17:
                    rsearch = re.search(j,file_source[1])
                    if rsearch is not None:
                        basic.success('Joomla version detected, version: ' + basic.bold + '1.7' + basic.cln)
                        return '1.7'
            ...
            for j in j10:
                rsearch = re.search(j,file_source[1])
                if rsearch is not None:
                    basic.success('Joomla version detected, version: ' + basic.bold + '1.0' + basic.cln)
                    return '1.0'
            ...
            for j in j25:
                rsearch = re.search(j,file_source[1])
                if rsearch is not None:
                    basic.success('Joomla version detected, version: ' + basic.bold + '2.5' + basic.cln)
                    return '2.5'

    # Detection using README file
    if version == '0':
        basic.statement('Detecting version from README file [Method 4 of 4]')
        readme_file = url + '/README.txt'
        readme_source = basic.getsource(readme_file, ua)
        if readme_source[0] == '1':
            regex_4 = re.findall(r'package to version (.*?)', readme_source[1])
            if regex_4 != []:
                basic.success('Joomla version detected, version: ' + basic.bold + regex_4[0] + basic.cln)
                return regex_4[0]
