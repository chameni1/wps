...
dir = os.path.dirname(os.path.abspath(__file__)).replace('cmseekdb','')[:-1]    
total_requests = 0
cstart = time.time()
...
def init_result_dir(url):
    ...
    if "http://" in url:
        ...
        print(' use the targetinp function!')
    if url.endswith('/'):
...
def update_brute_cache():
    ...
    global dir
    brute_dir = os.path.join(dir, "cmsbrute")
    ...
    if get_version[0] != '1':
        ...
        if my_version > serv_version:
            ...
            if update_me.lower() == 'y':
                ...
                try:
                    global dir
                    lock_file = os.path.join(dir, "/.git/index.lock")
